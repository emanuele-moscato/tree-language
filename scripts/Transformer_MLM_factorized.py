...
print('Masked language modeling with single symbols for l=4, 2**18 data trained on the new and fixed factorized model for k = 1, 2k epochs, linear readout.')
...
def setup(seq_len,embedding_size,vocab_size,n_layer,n_head,device,q,l,sigma,epsilon,seed,p,mask_frac,factorized_layer):
    ...
    #model_dir = './models/model_MLM_1mask_{}_{}_{:.2f}_{:.5f}_{}_{}_{}'.format(q,l,sigma,epsilon,seed,p,n_layer)
    model_dir = './models/model_MLM_1mask_factorizedNew_LinearReadout_{}_{}_{:.2f}_{:.5f}_{}_{}_{}_{}'.format(q,l,sigma,epsilon,seed,p,n_layer,factorized_layer)
    #model_dir = './models/model_MLM_{}_factorizedNew_{}_{}_{:.2f}_{:.5f}_{}_{}_{}_{}'.format(mask_frac,q,l,sigma,epsilon,seed,p,n_layer,factorized_layer)
...
N_val = int(1e4)
#P = 2**np.arange(9,15) # Number of training samples that will be used
#P = 2**np.arange(14,15)
#P = 2**np.arange(19,20)
P = 2**np.arange(18,19)

factorized_layers = factorized_layers[3:]
print(factorized_layers)
        ...
        for seed in seeds:
            for p in P:
                for factorized_layer in factorized_layers:
                    [_,_,_,_,_,xis_factorized,_,_] = np.load('./sim_data/labeled_data_fixed_factorizedNew_LARGE_{}_{}_{}_{:.5f}_{}.npy'.format(q,l,sigma,epsilon,factorized_layer),allow_pickle=True)
                    print(xis_factorized.shape)
                    torch.cuda.empty_cache()
                    #xi = xis_factorized[:,:,seed]
                    xi = xis_factorized[:,:,np.argmin(seeds==seed)] # For large dataset
                    ...
                    #np.save('./sim_data/Transformer_MLM_1mask_{}_{}_{:.2f}_{:.5f}_{}_{}_{}.npy'.format(q,l,sigma,epsilon,seed,p,n_layer),np.array([q,l,sigma,epsilon,seed,p,n_layer,training_history,embedding_size,p,N_val],dtype=object))
                    np.save('./sim_data/Transformer_MLM_1mask_factorizedNew_noLRscheduling_LinearReadout_{}_{}_{:.2f}_{:.5f}_{}_{}_{}_{}.npy'.format(q,l,sigma,epsilon,seed,p,n_layer,factorized_layer),np.array([q,l,sigma,epsilon,seed,p,n_layer,training_history,embedding_size,p,N_val],dtype=object))
                    #np.save('./sim_data/Transformer_MLM_{}_factorizedNew_noLRscheduling_{}_{}_{:.2f}_{:.5f}_{}_{}_{}_{}.npy'.format(mask_frac,q,l,sigma,epsilon,seed,p,n_layer,factorized_layer),np.array([q,l,sigma,epsilon,seed,p,n_layer,training_history,embedding_size,p,N_val],dtype=object))